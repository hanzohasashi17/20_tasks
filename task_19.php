...
if (isset($_FILES['uploads'])) {
    $uniqName = uniqid("upload/image_");
    $totalFiles = count($_FILES['uploads']['name']);

    $db = new PDO("mysql:host=localhost;dbname=crud", "root", "");
    for ($key = 0; $key < $totalFiles; $key++) {
        if (isset($_FILES['uploads']['name'][$key])) {
            try {
                $target = $uniqName . basename($_FILES['uploads']['name'][$key]);
                $tmp = $_FILES['uploads']['tmp_name'][$key];
                move_uploaded_file($tmp, $target);
                $sql = "INSERT INTO `images` (id, name) VALUES (null, '$target')";
                $db->exec($sql);
                            ...
                            <div class="form-group">
                                <form action="" enctype="multipart/form-data" method="post">
                                    <div class="form-group">
                                        ...
                                        <input type="file" id="simpleinput" class="form-control" multiple
                                               name="uploads[]">
                                    </div>
                                    ...
                                    foreach ($res as $row) {
                                        ...
                                        <div class="col-md-3 image">
                                            ...
                                            <a class="btn btn-danger" onclick="confirm('Вы уверены?');"
                                               href="task_18.php?delete=<?= $row['id'] ?>">Удалить</a>
                                        </div>
                                        <?php
                                    }
                                } catch (PDOException $e) {
